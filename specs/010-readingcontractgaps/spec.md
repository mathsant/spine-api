...
- [x] **Todos os cenários de aceitação acima passam** com testes automatizados (`vitest`): testes unitários e de integração para os dois endpoints novos e para o endpoint de listagem modificado (`GET /me/reading-sessions`), cobrindo filtro, ordenação, paginação e o objeto `book` embutido. — 574 testes verdes; specs novas: `list-book-reviews.service`, `list-popular-among-following.service`, `list-reading-sessions.service` (status/ordenação/cursor/`book`), `reading-session-cursor`, métodos novos de repositório, e testes de rota em `books.routes` / `reading-sessions.routes`.
- [x] **`docs/` atualizado e lint limpo**: `docs/openapi.yaml` cobre os 2 endpoints novos + `pageCount` + parâmetro `status` + nova ordenação + `book` embutido; `pnpm docs:lint` passa (2 warnings pré-existentes, no endpoint `/health`, sem relação com esta feature); `docs/flows/reading-flow.md`, `docs/flows/review-flow.md` e `docs/pagination-guide.md` atualizados; nomes finais de rota/schema enviados à sessão `spine-frontend`.
- [x] **Sem regressão nos fluxos existentes de leitura e review**: `books`, `reading-sessions`, `reviews` e `feed` mantêm o comportamento atual; suíte completa verde (`pnpm test` = 574); os únicos testes existentes ajustados são os da ordenação de `GET /me/reading-sessions`, onde o contrato mudou de propósito (RF-023/RF-027).
- [x] **Privacidade P6 garantida**: `GET /books/{olid}/reviews` e `GET /books/popular-among-following` só retornam/contabilizam dados de usuários seguidos com follow aprovado; testes explícitos: "excludes reviews from users the caller does not follow" e "does not count sessions of users the caller does not follow".
